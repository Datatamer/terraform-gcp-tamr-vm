...
# dataproc configuration template
data "template_file" "dataproc" {
  count    = var.tamr_dataproc_cluster_config == "" ? 1 : 0
  template = "${file("${path.module}/tamr_config.yaml.tmpl")}"
  vars = {
...
data "template_file" "tamr_config" {
  ...
  vars = {
    ...
    tamr_dataproc_cluster_config = local.dataproc_config,
    tamr_dataproc_bucket         = var.tamr_dataproc_bucket,
    # spark
    ...
    tamr_spark_properties_override = local.spark_properties,
    # sql
